     ...
     (let* ([xy (get-top-left-corner country i j)]
       ...
       (begin
        (send dc set-pen blue-pen)
         (iff (is-camp country i j) ; draw circle
            (begin
               ...
               (for* ([ii '(-1 1)]
                 ...
                 (iff (not (and (= i 2) (= j 2))) (iff (and (= (+ i ii) 2) (= (+ j jj) 2)) 
            ...
            ; else
            (begin
             (iff (is-base country i j) (send dc set-pen red-pen) (send dc set-pen blue-pen))        
             (send dc draw-rounded-rectangle (first xy) (second xy) (first ab) (second ab) ) ; draw rectangle
             (send dc set-pen blue-pen)
             )
         )
         (iff (< j 4) (send dc draw-line (first xy2) (second xy2) (first xyn) (second xyn)))
         (iff (< i 5) (send dc draw-line (first xy3) (second xy3) (first xyd) (second xyd)))
         )))
