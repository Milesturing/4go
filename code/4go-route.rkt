...
(provide route-list) ; (route-list occupied? country row col is-labor? country2 row2 col2)
...
(define (route-list occupied? country row col is-labor? country2 row2 col2) ; chess = 30 to check if it is labor
  ...
  (iff (and (on-rail country row col) (on-rail country2 row2 col2))
     (set! result
     (iff is-labor?
  ...
  (iff (> (length result) 2)
      (iff  (eval (cons 'or (map (lambda (x) (apply occupied? x)) (drop-right (cdr result) 1)) )) 
          (set! result null) 
      )
   )    
  ; return
